...
<li>
	<div class="flex items-center gap-2">
		<Tooltip.Root>
			...
			<Tooltip.Content>Node is {node.ready ? "Online" : "Offline"}.</Tooltip.Content>
		</Tooltip.Root>

		<p class="text-sm font-semibold">{node.name}</p>
	</div>

	{#if expanded && node.address}
		<div class="mt-1 flex items-center gap-4" transition:fly={{ y: -5 }}>
			<p class="align-center inline-flex items-center gap-1 text-sm text-muted-foreground">
				<Globe class="h-4 w-4" />{node.address}
			</p>
		</div>
	{/if}
</li>
