<script lang="ts">
	...
	const getItemLink = (itemId?: string, serverId?: string | null) => {
		return `https://jellyfin.${domain}/web/index.html#/details?id=${encodeURIComponent(itemId ?? "")}&serverId=${encodeURIComponent(serverId ?? "")}`;
	};
...
<Carousel.Item class="">
	<div class="p-1">
		<a href={getItemLink(Id, ServerId)} class="contents" target="_blank">
			<Card.Root class="relative overflow-hidden transition-all hover:border-primary/30">
				<canvas
				...
				>
					...
					<p
						class="align-center mt-3 flex items-center justify-center gap-1 text-sm font-semibold uppercase text-muted-foreground md:ml-auto md:mt-0"
					>
						Stream now
