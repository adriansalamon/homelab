...
let
    ...
    forEach (attrValues nixosConfigurations) (
      ...
      forEach (attrValues hostCfg.config.services.restic.backups) (
        ...
        optional backupCfg.hetznerStorageBox.enable (
          ...
          // {
            sshPrivateKeyFile =
              hostCfg.config.age.secrets.${backupCfg.hetznerStorageBox.sshSecretName}.rekeyFile;
          }
        )
      )
    )
  ...
  boxSubuserToMainUser = flip mapAttrs boxesBySubuser (
    _: boxes: head (unique (forEach boxes (box: box.mainUser)))
  );

  boxSubuserToPrivateKeys = flip mapAttrs boxesBySubuser (
    _: boxes: unique (forEach boxes (box: box.sshPrivateKeyFile))
  );

  # Any subuid that has more than one path in use
  boxSubuserToPaths = flip mapAttrs boxesBySubuser (_: boxes: unique (forEach boxes (box: box.path)));
  ...
  authorizeResticCommand = privateKey: ''
    (
      echo -n 'command="rclone serve restic --stdio ./repo" '
      KEYFILE=$(mktemp)
      PATH="$PATH:${pkgs.age-plugin-yubikey}/bin" ${pkgs.rage}/bin/rage -d -i ${decryptIdentity} ${escapeShellArg privateKey} > "$KEYFILE"
      ssh-keygen -f "$KEYFILE" -y
      rm "$KEYFILE"
    ) >> "$TMPFILE"
  '';
  ...
  program = getExe (
    pkgs.writeShellApplication {
      ...
      text = ''
        ...
        echo
        echo "[33mPlease visit https://console.hetzner.com/projects/xxxx/storage-boxes and make sure"
