...
let
  ...
  backupCfg = nomadConfig.config.backups.seaweedfs;
    ...
    group.backup = {
      ...
      task.backup-seaweedfs = {
        ...
        env = {
          ...
          # SeaweedFS S3 endpoint via Consul
          S3_ENDPOINT = "https://s3.local.${globals.domains.main}";
        };

        config = {
          # Use Alpine with rclone and restic
          image = "alpine:3.23";
          entrypoint = [ "/bin/sh" ];
          args = [
            "-c"
