...
in
{
  age.secrets = {
    ...
    patroni-superuser-password = {
      inherit (nodes.demeter.config.age.secrets.patroni-superuser-password) rekeyFile;
      owner = config.services.patroni.user;
    };

    patroni-replication-password = {
      inherit (nodes.demeter.config.age.secrets.patroni-replication-password) rekeyFile;
      owner = config.services.patroni.user;
    };

    patroni-consul-token = {
      rekeyFile = config.node.secretsDir + "/patroni-consul-token.age";
      owner = config.services.patroni.user;
    };
  };
