# From <https://github.com/oddlama/nixos-extra-modules/blob/12b9f46/lib/net.nix>
inputs: _final: prev:
let
  inherit (prev.lib)
    all
  ...
  lib = recursiveUpdate prev.lib {
    ...
    net = recursiveUpdate (removeAttrs libNet.net [ "types" ]) {
      cidr = rec {
        ...
        # withCidr :: ip -> cidr -> cidr
        #
        # Returns the ip cidr with the given cidr length
        #
        # Examples:
        #
        # > net.cidr.withCidr "192.168.1.100" 192.168.1.100/24
        # "192.168.1.100/24"
        withCidr = ip: cidr: "${ip}/${toString (libNet.net.cidr.length cidr)}";
